...
export class FleetmaticsComponent implements OnInit {
  ...
  constructor(
    public databotService: DatabotService,
    private route: ActivatedRoute
  ) {
    if (
      localStorage.getItem("username") == "melrosepark" ||
      localStorage.getItem("username") == "Melrosepark"
    ) {
  ...
  ngOnInit() {
    ...
    const searchfromdate =
      this.route.snapshot.queryParamMap.get("searchfromdate");
  ...
  ScoreChart(score, rash) {
    this.scoreChartOptions = {
      ...
      title: {
        text:
          '<p style=" font-size: 20px">' + score + "%</p>" + "<br><p>Score</p>",
        align: "center",
        verticalAlign: "middle",
        y: 3,
        style: {
          ...
          if (item["personName"] == this.driver) {
            ...
            startmarker.addListener("mouseover", function () {
              infowindow = new google.maps.InfoWindow({
                ...
                content:
                  '<b ><p style="color:white; background:black;text-weight:bold; padding:5px;">' +
                  "Driver Name&nbsp;:&nbsp;" +
                  item["personName"] +
                  "</p></b>" +
                  // tslint:disable-next-line: max-line-length
                  '<b style="display:inline-block;"><p style="color:black;text-weight:bold; ">' +
                  ' <img src="../assets/fuel.jpg" style="width: 22px;"> Fuel&nbsp;&nbsp:&nbsp;' +
                  item["fuelLevel"] +
                  "</p></b>" +
                  // tslint:disable-next-line: max-line-length
                  '<b style="display:inline-block;"><p style="color:black;text-weight:bold; margin-left:30px;">' +
                  ...
                  "</p></b><br>" +
                  // tslint:disable-next-line: max-line-length
                  '<b style=""><p style="color:black;float:left;margin-left: 6px;font-family: sans-serif;">' +
                  '<img src="../assets/odometer.png" style="width:18px">&nbsp;Odo&nbsp;:&nbsp;' +
                  item["virtualOdo"] +
                  "</p></b>" +
                  // tslint:disable-next-line: max-line-length
                  '<b style="display:inline-block; float:right;font-family: sans-serif;margin-right: 19px;"><p style="color:black;text-weight:bold;">' +
    ...
    this.databotService.getVehicleAlerts(this.getParams()).subscribe((data) => {
      for (let item of data["data"]["alerts"]) {
        if (item["deviceNumber"] == this.imei) {
          ...
          this.VIN = item["vin"];
          let alerttime = moment(item["alertDateTime"]).format(
            "YYYY-MM-DD  hh:mm:ss A"
          );
          this.alertArray.push({
            "Device Serial Number": item["deviceSerialNumber"],
            "Device Type": item["deviceTypeDescription"],
            Alert: item["alertShortDesc"],
            "Alert Description": item["alertDesc"],
            "Alert Time": alerttime,
          });
          this.columns = Object.keys(this.alertArray[0]);
          // this.columns.splice(0, 1);
          this.alertitemResource = new DataTableResource(this.alertArray);
          this.alertitemResource
            .count()
            .then((count: any) => (this.itemCount = count));
  ...
  getIdlingEvents(stops) {
    ...
    for (var item in stops) {
      ...
      var momentString = momentObj.format("MMM DD, YYYY");
      if (
        stops[item]["deviceNbr"] == this.imei &&
        stops[item]["stopType"] == "Engine Off"
      ) {
        ...
        let tripStart = moment(stops[item]["beginDate"]).format(
          "YYYY-MM-DD hh:mm:ss A"
        );
        let tripEnd = moment(stops[item]["endDate"]).format(
          "YYYY-MM-DD hh:mm:ss A"
        );
        let location =
          stops[item]["street"] +
          ...
          stops[item]["city"] +
          "," +
          stops[item]["stateCode"] +
          "," +
          stops[item]["countryCode"] +
          "," +
          stops[item]["postalCode"];
        ...
        this.stopitemResource
          .count()
          .then((count: any) => (this.stopitemCount = count));
        this.stopreloadItems(this.param);
      }
      if (
        stops[item]["deviceNbr"] == this.imei &&
        stops[item]["stopType"] == "Idling"
      ) {
        ...
        this.idleitemResource
          .count()
          .then((count: any) => (this.idleitemCount = count));
    ...
    for (let item of trips) {
      ...
      if (item["driverName"] == this.driver) {
        ...
        this.tripArray.push({
          ...
          "Duration(Mins)": item["durationMinutes"],
        ...
        this.tripitemResource
          .count()
          .then((count: any) => (this.tripitemCount = count));
  ...
  getRoute() {
    this.databotService
      .getVehicleRouteLocations(this.getParams(), this.imei)
      .subscribe((data) => {
  ...
  loadRoute() {
    ...
    var centerLatlng = new google.maps.LatLng(
      routes[Math.round(this.locations.length / 2)]
    );
  ...
  stopreloadItems(params: any) {
    this.stopitemResource
      .query(params)
      .then((items: any) => (this.stopitems = items));
  }
  idlereloadItems(params: any) {
    this.idleitemResource
      .query(params)
      .then((items: any) => (this.idleitems = items));
  }
  tripreloadItems(params: any) {
    this.tripitemResource
      .query(params)
      .then((items: any) => (this.tripitems = items));
  }
  ...
  rowIdleClick(event) {
    ...
    geocoder.geocode({ address: loc }, function (res, status) {
      if (status == "OK") {
        ...
        map = new google.maps.Map(
          document.getElementById("idleminimap"),
          mapOptions
        );
        var startmarker = new google.maps.Marker({
          position: new google.maps.LatLng(startlatitude, startlongitude),
  ...
  rowStopClick(event) {
    ...
    geocoder.geocode({ address: loc }, function (res, status) {
      if (status == "OK") {
        ...
        map = new google.maps.Map(
          document.getElementById("stopminimap"),
          mapOptions
        );
        var startmarker = new google.maps.Marker({
          position: new google.maps.LatLng(startlatitude, startlongitude),
