...
export class FleetmaticusescomponentComponent implements OnInit, OnDestroy {
  ...
  loadData() {
    ...
    var body = { username: "info@dataagile.com", password: "conquest" };
    this.databotService
      .getVehicleLocations(this.getParams())
      .subscribe((data) => {
  ...
  loadmap() {
    var mapOptions = {
      center: new google.maps.LatLng(
        this.mapdata[5].latitude,
        this.mapdata[5].longitude
      ),
      zoom: 14,
      mapTypeId: google.maps.MapTypeId.ROADMAP,
    };
  ...
  updatemarkers(fromdate, todate) {
    ...
    for (var i = 0; i < this.vehicleinfo.length; i++) {
      var latLng = new google.maps.LatLng(
        this.vehicleinfo[i].latitude,
        this.vehicleinfo[i].longitude
      );
      ...
      if (this.vehicleinfo[i]["speed"] == "0") {
        ...
        if (this.vehicleinfo[i].heading == "N") {
          ...
          this.vehicleinfo[i].heading == "E" ||
          this.vehicleinfo[i].heading == "NE" ||
          this.vehicleinfo[i].heading == "SE"
        ) {
          icon = "../../assets/right.png";
          size = new google.maps.Size(15, 15);
        } else if (
          this.vehicleinfo[i].heading == "W" ||
          this.vehicleinfo[i].heading == "NW" ||
          this.vehicleinfo[i].heading == "SW"
        ) {
        ...
        google.maps.event.addListener(marker, "mouseover", function (e) {
          ...
          geocoder.geocode({ location: latlong }, function (res, status) {
            ...
            infoWindow.setContent(
              '<div class="row" style="margin:0px"><div div class="row" style="margin:0px;background:black;width:360px"><div class="col-md-7" style="padding:0px"><span style="color:white;font-family: sans-serif; background:black;text-weight:bold;">' +
                '<i class="fa fa-map-marker" aria-hidden="true"></i> Current Location:' +
                currentlocation +
                "</span></div>" +
                // tslint:disable-next-line: max-line-length
                '<div class="col-md-5" style="padding:0px;"><span style="color:white; background:black;text-weight:bold;font-family: sans-serif;">' +
                "Driver Name:" +
                data["personName"] +
                "</span></div></div><br>" +
                // tslint:disable-next-line: max-line-length
                '<div class="row" style="margin:0px"><div class="col-md-4" style="padding:0px"><b style=""><span style="color:black;float:left;font-family: sans-serif;">' +
                '<img src="../assets/fuel.jpg" style="width: 22px;">&nbsp;&nbsp;Fuel&nbsp;:&nbsp;' +
                data["fuelLevel"] +
                "</span></b></div>" +
                // tslint:disable-next-line: max-line-length
                '<div class="col-md-4" style="padding:0px"><b style="display:inline-block;" ><span style="color:black;text-weight:bold;font-family: sans-serif;">' +
                '<i class="fa fa-battery-quarter" aria-hidden="true"></i> Battery&nbsp;:&nbsp;' +
                data["battery"] +
                "V" +
                "</span></b></div>" +
                // tslint:disable-next-line: max-line-length
                '<div class="col-md-4" style="padding:0px"><b style="display:inline-block; float:right;font-family: sans-serif;"><span style="color:black;text-weight:bold">' +
                '<img src="../assets/speed.jpg"> Speed&nbsp;:&nbsp;' +
                data["speed"] +
                "mph" +
                "</span></b>" +
                "</div></div>" +
                // tslint:disable-next-line: max-line-length
                '<div class="row" style="margin:0px"><div class="col-md-4" style="padding:0px"><b style=""><span style="color:black;float:left;font-family: sans-serif;">' +
                '<img src="../assets/odometer.png" style="width:18px">&nbsp;Odo&nbsp;:&nbsp;' +
                Math.round(data["virtualOdo"]) +
                "</span></b></div>" +
                // tslint:disable-next-line: max-line-length
                '<div class="col-md-8" style="padding:0px;margin-left: -50px;"><b style="display:inline-block; float:right;font-family: sans-serif;"><span style="color:black;text-weight:bold;">' +
                '<img src="../assets/battery.png" style="width:16px"> Device Status&nbsp;:&nbsp;' +
                deviceStatus +
                "</span></b></div></div></div>"
            );
            infoWindow.open(this.map, marker);
    ...
    for (let item of this.mapdata) {
      ...
      if (this.username == "melrosepark") {
        if (item["fleetId"] == 129900) {
          ...
          this.vehicleTrackerInfo.push({
            Driver: item["personName"],
            VIN: item["vin"],
            Speed: item["speed"],
            Status: status,
            icon: icon,
  ...
  getIdlingEvents(stops) {
    ...
    for (var item in stops) {
      let today = moment(Date.now()).format("MMM DD, YYYY");
      let todayfromdata = moment(stops[item]["beginDate"]).format(
        "MMM DD, YYYY"
      );
  ...
  IdlereloadItems(params: any) {
    this.IdleitemResource.query(params).then(
      (items: any) => (this.Idleitems = items)
    );
  }
  ...
  getTotalDriveTime(trips, idlingtime, stoptime) {
    ...
    for (let item of trips) {
      ...
      this.unauthorizedMiles =
        this.unauthorizedMiles + item["unauthorizedMiles"];
  ...
  DTreloadItems(params: any) {
    this.DTitemResource.query(params).then(
      (items: any) => (this.DTitems = items)
    );
  }
  ...
  searchingBydate() {
    ...
    if (this.username == "melrosepark") {
      ...
      this.databotService
        .getMelroseInfobyDates(this.searchfromdate, this.searchtodate)
        .subscribe((data) => {
          console.log(data);
          this.flaskinfo = data;
        });
      ...
      this.databotService
        .getAllInfobyDates(this.searchfromdate, this.searchtodate)
        .subscribe((data) => {
          console.log(data);
          this.flaskinfo = data;
        });
  ...
  vehicleDDChange(event) {
    ...
    var $this = this;
    var latlng = new google.maps.LatLng(
      vhehicleobj[0].latitude,
      vhehicleobj[0].longitude
    );
    var currentlocation;
    geocoder.geocode({ location: latlng }, function (res, status) {
      if (status == "OK") {
  ...
  getTotalAlerts() {
    ...
    this.databotService.getVehicleAlerts(this.getParams()).subscribe((data) => {
      ...
      for (let item of this.alertTypes) {
        this.totalDrivers.push({
          driver: item["firstName"],
          alerttype: item["alertCode"],
        });
      }
  ...
  getIdleEventData(stops) {
      ...
      for (let item of stops) {
        if (item["lastName"] == "PW") {
          if (item["stopType"] == "Idling") {
            ...
            let tripId = item["id"];
            let tripStart = moment(item["beginDate"]).format(
              "YYYY-MM-DD  hh:mm:ss A"
            );
            let tripEnd = moment(item["endDate"]).format(
              "YYYY-MM-DD hh:mm:ss A"
            );
    ...
    // this.columns.splice(0, 1);
    this.IdleitemResource = new DataTableResource(this.idleArray);
    this.IdleitemResource.count().then(
      (count: any) => (this.IdleitemCount = count)
    );
    this.IdlereloadItems(this.Idleparam);
  }
  ...
  getMilesInfo() {
    this.databotService
      .getVehicleTrips(this.getParams())
      .subscribe((result) => {
        this.trips = result["data"]["trips"];
        // console.log(this.trips);
  ...
  getMilesEventData(trips) {
    ...
    if (this.username == "melrosepark") {
      for (let item of trips) {
        if (
          item["driverName"] == "MP1 PW" ||
          item["driverName"] == "MP2 PW" ||
          item["driverName"] == "MP3 PW" ||
          item["driverName"] == "MP4 PW" ||
          item["driverName"] == "MP5 PW" ||
          item["driverName"] == "MP6 PW" ||
          item["driverName"] == "MP7 PW" ||
          item["driverName"] == "MP8 PW" ||
          item["driverName"] == "MP9 PW" ||
          item["driverName"] == "MP10 PW"
        ) {
          count = count + item["authorizedMiles"];
          let startTime = moment(item["startDateTime"]).format(
            "YYYY-MM-DD hh:mm:ss A"
          );
          let endTime = moment(item["endDateTime"]).format(
            "YYYY-MM-DD hh:mm:ss A"
          );
          this.DTArray.push({
            ...
            "Authorized Miles": item["authorizedMiles"],
            "Total Miles": item["distanceMiles"],
          });
        }
      }
    } else {
    ...
    this.DTcolumns = Object.keys(this.DTArray[0]);
    this.DTitemResource = new DataTableResource(this.DTArray);
    this.DTitemResource.count().then(
      (count: any) => (this.DTitemCount = count)
    );
    this.DTreloadItems(this.DTparam);
  }
  ...
  getAlertMelInfo() {
    ...
    this.databotService.getalertArrayMelinfo().subscribe((data) => {
      ...
      this.speeditemResource
        .count()
        .then((count: any) => (this.speeditemCount = count));
      ...
      this.HBitemResource.count().then(
        (count: any) => (this.HBitemCount = count)
      );
      ...
      this.ACCitemResource.count().then(
        (count: any) => (this.ACCitemCount = count)
      );
  ...
  speedreloadItems(params: any) {
    this.speeditemResource
      .query(params)
      .then((items: any) => (this.speeditems = items));
  }
  HBreloadItems(params: any) {
    this.HBitemResource.query(params).then(
      (items: any) => (this.HBitems = items)
    );
  }
  ACCreloadItems(params: any) {
    this.ACCitemResource.query(params).then(
      (items: any) => (this.ACCitems = items)
    );
  }
  ...
  getAlertInfoByDates(searchfromdate, searchtodate) {
    ...
    this.ACCArray = [];
    this.databotService
      .getalertArrayinfoByDates(searchfromdate, searchtodate)
      .subscribe((data) => {
        // console.log(data);
        this.speedArray = data["HIGH_SPEED"];
        this.HBArray = data["HARSH_BRAKING"];
  ...
  TruckInfo(item) {
    ...
    geocoder.geocode({ location: latLng }, function (res, status) {
      if (status == "OK") {
        ...
        map = new google.maps.Map(
          document.getElementById("truckminimap"),
          mapOptions
        );
        var startmarker = new google.maps.Marker({
          position: new google.maps.LatLng(lat, long),
