...
export class ManagementComponent implements OnInit {
  ...
  getChartsData() {
    this.dataService.getEnegryAPi().subscribe(
      (data) => {
        console.log(data);
  ...
  getEPCChart(data) {
    ...
    names.map((item, index) => {
      series.push(
        { name: names[index], color: color[index], data: totalcases[index] },
        { name: "souece", color: "#C25763", data: totalcases2[index] }
      );
    });
  ...
  getMPCChart(data) {
    ...
    data.forEach(function (a) {
      ...
      if (!hash[key]) {
        hash[key] = {
          count: 0,
          data: { name: a.source, plants: 0, id: 0, y: 0 },
        };
        result.push(hash[key].data);
      }
      hash[key].data.plants += a.plants;
    ...
    names.map((item, index) => {
      series.push(
        { name: names[index], color: color[index], data: totalcases[index] },
        { name: "Category", color: "#000000", data: totalcases2[index] },
        { name: "Source", color: "#C25763", data: totalcases3[index] }
      );
    });
