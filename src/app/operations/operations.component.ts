...
export class OperationsComponent implements OnInit {
  ...
  getconstructiondata() {
    this.dataService.getConstructionsApi().subscribe(
      (data) => {
  ...
  getpublicdata() {
    this.dataService.getPublicworksApi().subscribe(
      (data) => {
  ...
  getHistoric(data) {
    ...
    data.forEach(function (a) {
      var key = ["month"]
        ...
        .join("|");
      if (
        a.inspectionsPerformed === undefined ||
        a.inspectionsPerformed === null
      ) {
        return;
      }
      if (!hash[key]) {
  ...
  getProjectwithin(data) {
    ...
    data.forEach(function (a) {
      ...
      if (!hash[key]) {
        hash[key] = {
          count: 0,
          data: { name: a.month, b: 0, c: 0, d: 0, e: 0 },
        };
        result.push(hash[key].data);
      }
  ...
  getAveragecustomer(data) {
    ...
    data.forEach(function (a) {
      ...
      if (!hash[key]) {
        hash[key] = {
          count: 0,
          data: { name: a.projectName, budgetAllocation: 0 },
        };
        result.push(hash[key].data);
      }
    ...
    result.map((item, index) => {
      series.push({
        ...
        color: this.treemapColors[index].color,
        backgroundColor: this.treemapColors[index].backgroundColor,
      });
    });
    console.log("chart", series);
  ...
  getAveragewithin(data) {
    ...
    result.map((item, index) => {
      series.push({
        name: item.name,
        value: item.budgetAllocation,
        color: this.treeColors[index].color,
        backgroundColor: this.treeColors[index].backgroundColor,
      });
    });
